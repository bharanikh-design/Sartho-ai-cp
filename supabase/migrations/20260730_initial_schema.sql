...
-- Reproducible foundation for new/local Supabase projects. The existing live
-- project predates migration tracking and must use the separate reconciliation
-- package instead of replaying this migration chain.
...
create policy "own target lanes" on public.target_lanes for all to authenticated
  using (auth.uid() = user_id) with check (auth.uid() = user_id);

drop policy if exists "own career roles" on public.career_roles;
create policy "own career roles" on public.career_roles for all to authenticated
  using (auth.uid() = user_id) with check (auth.uid() = user_id);

drop policy if exists "own evidence" on public.evidence_items;
create policy "own evidence" on public.evidence_items for all to authenticated
  using (auth.uid() = user_id) with check (auth.uid() = user_id);
...
create policy "own job requirements" on public.job_requirements for all to authenticated
  using (
    exists (
