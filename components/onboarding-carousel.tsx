...
export function OnboardingCarousel({user}:{user:User}){
 ...
 return <div className="tour-backdrop" role="dialog" aria-modal="true" aria-labelledby="tour-title"><div className="tour-cinema" ref={panelRef}>
   <div className="tour-topline"><span className="tour-count">{index+1} of {slides.length}</span><button type="button" className="tour-close" onClick={()=>void finish()} aria-label="Close product tour">×</button></div>
   <button type="button" className="tour-arrow tour-arrow-left" onClick={()=>setIndex(i=>Math.max(0,i-1))} disabled={index===0} aria-label="Previous">‹</button><div className="tour-stage" key={index}><section className="tour-copy"><span className="tour-eyebrow">{slide.eyebrow}</span><h1 id="tour-title">{slide.title}</h1><p>{slide.description}</p><strong className="tour-outcome">{slide.outcome}</strong><span className="tour-route">{slide.route}</span></section>
